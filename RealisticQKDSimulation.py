# 6 -6-2025
# Realizable QKD generation transmitted at Zenith passes
# Generates Random QKD Key Bits → Cryptographic-grade random binary keys.
# ✔ Implements Key Verification → Parity check ensures secure key transmission.
# ✔ Secure Key Storage → Encrypts keys using Fernet encryption before saving.
# ✔ Schedules QKD sessions during zenith passes → Allocates optimal time slots for communication.
# ✔ Rejects Invalid Keys → Ensures only verified keys are transmitted.
# ✔ Displays Graphs & Key Data for Demo → Allows audience to see results dynamically.
import numpy as np
